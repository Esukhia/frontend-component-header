...
const useReleaseNearFooter = footerSelectorProp => {
  ...
  useEffect(() => {
    ...
    const checkFooterVisibility = instant => {
      ...
      const inView = !!footerNode && window.scrollY > 0 && footerNode.getBoundingClientRect().top < window.innerHeight;
      // Return the previous object when unchanged so setState can bail out.
      setState(previous => previous.inView === inView && previous.instant === instant ? previous : {
        inView,
        instant
      });
    };
    ...
    const removalContainsFooter = removedNodes => {
      for (let i = 0; i < removedNodes.length; i += 1) {
        ...
        if (removed === footerNode || removed.nodeType === Node.ELEMENT_NODE && removed.contains(footerNode)) {
          return true;
        }
      }
      return false;
...
//# sourceMappingURL=useReleaseNearFooter.js.map