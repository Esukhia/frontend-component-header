import getInitials, { matchActiveLanguage, readLanguageCookie, resolveHeaderLanguages } from './utils';
...
describe('resolveHeaderLanguages', () => {
  ...
  it('ignores the name the config API sends', () => {
    const [tibetan] = resolveHeaderLanguages([{
      code: 'bo',
  ...
  it('skips entries with no usable code', () => {
    expect(resolveHeaderLanguages(['fr', null, {
      name: 'no code'
    }, '  '])).toEqual([{
      code: 'fr',
      native: 'Français'
...
describe('matchActiveLanguage', () => {
  ...
  it('does not match a different region of the same language', () => {
    expect(matchActiveLanguage('zh-tw', [{
      code: 'zh-cn'
    }])).toBe('zh-cn');
    expect(matchActiveLanguage('zh-tw', [{
      code: 'en'
