 ...
 */
const SiteLoggedOutItems = _ref => {
  let {
    items
  } = _ref;
  return items.map((item, index) => {
    const isPrimary = item.variant ? item.variant === 'signin' : items.length > 1 && index === items.length - 1;
 ...
 */
const itemsWithVariantWarning = function (props, propName, componentName) {
  ...
  if (Array.isArray(items) && items.length === 1 && !items[0].variant) {
    return new Error(`Invalid prop \`${propName}\` supplied to \`${componentName}\`: a single entry with no ` + '`variant` renders as the outlined (register) style by default, since "last of the list" and ' + '"the only item" look identical from position alone. Set `variant: \'signin\'` explicitly if ' + 'this one item should be the primary (filled) action instead.');
  }
  return null;
};
...
//# sourceMappingURL=SiteLoggedOutItems.js.map