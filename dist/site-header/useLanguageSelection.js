...
import { getLocale, handleRtl, useIntl, LOCALE_CHANGED } from '@edx/frontend-platform/i18n';
 ...
 */
const useLanguageSelection = function () {
  let {
    onSwitched
  ...
  const languages = useMemo(() => resolveHeaderLanguages(RELEASED_LANGUAGES), [RELEASED_LANGUAGES]);

  // selectedCode wins so the tick moves immediately on switch, without waiting on the cookie.
  const activeCode = selectedCode ?? matchActiveLanguage(readLanguageCookie(LANGUAGE_PREFERENCE_COOKIE_NAME) || intl.locale, languages);
...
//# sourceMappingURL=useLanguageSelection.js.map